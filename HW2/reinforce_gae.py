...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        state_value = [action.value for action in saved_actions]
        advs = GAE(gamma, 0.95, None)(self.rewards, state_value, False)

        for (log_prob, state_value), adv, r in zip(saved_actions, advs, returns):
...
def train(lr=0.01):
    ...
    for i_episode in count(1):
        ...
        writer.add_scalar('Training/training loss', loss, i_episode)
        writer.add_scalar('Training/learning rate', scheduler.get_lr()[-1], i_episode)
        writer.add_scalar('Training/ewma reward', ewma_reward, i_episode)
        writer.add_scalar('Training/episode reward', ep_reward, i_episode)
        ...
        if ewma_reward > 120:
            if not os.path.isdir("./preTrained"):
                os.mkdir("./preTrained")
            torch.save(model.state_dict(), './preTrained/LunarLander0.95_{}.pth'.format(lr))
...
if __name__ == '__main__':
    ...
    train(lr)
    test(f'LunarLander0.95_{lr}.pth')
