...
class DDPG(object):
    def __init__(self, num_inputs, action_space, gamma=0.995, tau=0.0005, hidden_size=128, lr_a=1e-3, lr_c=5e-3):
    ...
    def update_parameters(self, batch):
        ...
        q_target = reward_batch.view(-1, 1) + self.gamma * next_q * mask_batch.view(-1, 1)
...
def train(env_name):    # last episodes=300 hidden_size=256 batch_size=256 lra=1e-3 lrc=1e-3
    num_episodes = 200
    gamma = 0.99
    tau = 0.005
    hidden_size = 128
    ...
    for i_episode in range(num_episodes):
        ...
        while True:
            ...
            if len(memory) >= batch_size:
                for a in range(updates_per_step):
                    transitions = memory.sample(batch_size)
                    batch = Transition(*zip(*transitions))
                    cri_loss, act_loss = agent.update_parameters(batch)
        ...
        actor_loss = np.mean(act_losses)
        critic_loss = np.mean(cri_losses)
        ...
        if i_episode % print_freq == 0:
            ...
            print("Episode: {}, length: {}, reward: {:.2f}, ewma reward: {:.2f}, actor loss: {:.2f}, critic loss: {:.2f}".format(i_episode, t, rewards[-1], ewma_reward, actor_loss, critic_loss))

            # Use Tensorboard to record the behavior of implementation
            writer.add_scalar('Training/actor loss', actor_loss, i_episode)
            writer.add_scalar('Training/critic loss', critic_loss, i_episode)
            writer.add_scalar('Training/ewma reward', ewma_reward, i_episode)
